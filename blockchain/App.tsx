...
function App() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (loginId.trim()) {
      ...
      // 💰 Add 50 HP to new users by checking if they already have a balance
      const existingBalance = blockchain.getBalanceOfAddress(loginId);
      if (existingBalance === 0) {
        blockchain.addBalance(loginId, 50); // Add default 50 HP
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              {currentUser ? (
                <>
                  <div className="flex items-center bg-hp-blue-dark/50 px-4 py-2 rounded-lg mr-4">
                ...
                <form onSubmit={handleLogin} className="flex space-x-2">
                  <input
                    ...
                    className="bg-white rounded px-4 py-2 border border-hp-gray-medium focus:border-hp-blue-light outline-none transition-colors text-hp-gray-dark"
                    value={loginId}
                    onChange={(e) => setLoginId(e.target.value)}
                  />
                  <button
                    type="submit"
                    className="flex items-center bg-white hover:bg-hp-gray-light text-hp-blue px-4 py-2 rounded-lg transition-colors border border-hp-blue"
                  >
                    <LogIn className="h-5 w-5 mr-2 text-hp-blue" />
                    Login
                  </button>
                </form>
              )}
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {currentUser && (
          <>
            ...
            {/* User Transactions */}
            <div className="bg-white rounded-lg p-6 mb-8 border border-hp-gray-medium shadow-sm">
              <h2 className="text-xl font-bold mb-4 flex items-center text-hp-blue">
                <ArrowRightLeft className="h-6 w-6 mr-2" />
                My Transactions
              </h2>
              {getUserTransactions().map((tx, index) => (
                <div key={index} className={`bg-hp-gray-light rounded p-4 mt-2 border ${tx.status === 'failed' ? 'border-red-500' : 'border-hp-gray-medium'
                  }`}>
        ...
        <div className="bg-hp-gray-light rounded-lg p-6 mb-8 border border-hp-gray-medium">
          ...
          {filteredTransactions.length > 0 && (
            <div className="mt-4">
              <h3 className="text-lg font-semibold mb-2 text-hp-blue">Filtered Transactions</h3>
        ...
        {failedTransactions.length > 0 && (
          <div className="bg-white rounded-lg p-6 mb-8 border border-red-500/20 shadow-sm">
            <h2 className="text-xl font-bold mb-4 flex items-center text-red-500">
              <XCircle className="h-6 w-6 mr-2" />
              Failed Transactions
            </h2>
            {failedTransactions.map((tx, index) => (
              <div key={index} className="bg-hp-gray-light rounded p-4 mt-2 border border-red-500/50 shadow-sm">
                <div className="grid grid-cols-3 gap-2 text-sm">
                  ...
                  <div className="text-right font-mono text-red-500 font-bold">
                    {tx.amount} HP Coin
                  </div>
                </div>
                <div className="mt-2 text-sm text-red-400">
            ...
            <div key={block.index} className="bg-white rounded-lg p-6 border border-hp-gray-medium shadow-sm">
              ...
              <div className="mt-4">
                ...
                {block.transactions.map((tx, index) => (
                  <div key={index} className="bg-hp-gray-light rounded p-4 mt-2 border border-hp-gray-medium shadow-sm">
                    <div className="grid grid-cols-3 gap-2 text-sm">
                      <div className="truncate">
                        <span className="text-hp-gray-dark/60">From: </span>
                        <span className="font-mono">{tx.sender}</span>
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="bg-hp-gray-light p-4 rounded-lg border border-hp-gray-medium">
              ...
              <p className="font-bold text-hp-blue text-xl">5 HP Coin</p>
            </div>
            <div className="bg-hp-gray-light p-4 rounded-lg border border-hp-gray-medium">
              <div className="flex items-center mb-2">
